...
public class ResearchDashboardScraper : IDisposable
{
    ...
    public List<ResearchItem> ScrapeResearch()
    {
        ...
        var gridRoot = WaitForGridRootWithProgress(TimeSpan.FromSeconds(60));
        if (gridRoot == null)
        {
            Console.WriteLine($"No grid appeared within 60s for {assetClassTabText} — treating as empty.");
    ...
    public List<ResearchItem> ScrapeFutures()
    {
        ...
        var futureGridRoot = WaitForGridRootWithProgress(TimeSpan.FromSeconds(60));
        if (futureGridRoot == null)
        {
            Console.WriteLine("  Future grid never appeared within 60s — skipping Future for this cycle.");
            return new List<ResearchItem>();
        }
    ...
    public void EnrichWithDetails(ResearchItem item)
    {
        ...
        if (gridRoot == null)
        {
            Console.WriteLine($"  Grid never appeared within 60s — skipping detail fetch for {item.Symbol}.");
            return;
        }
