...
public class ResearchDashboardScraper : IDisposable
{
    ...
    public List<ResearchItem> ScrapeResearch()
    {
        ...
        var gridRoot = WaitForGridRootWithProgress(TimeSpan.FromSeconds(100));
        if (gridRoot == null)
        {
            Console.WriteLine($"No grid appeared within 100s for {assetClassTabText} — treating as empty.");
    ...
    public List<ResearchItem> ScrapeFutures()
    {
        ...
        var futureGridRoot = WaitForGridRootWithProgress(TimeSpan.FromSeconds(100));
        if (futureGridRoot == null)
        {
            Console.WriteLine("  Future grid never appeared within 100s — skipping Future for this cycle.");
            return new List<ResearchItem>();
        }
    ...
    /// timeout. Returns null instead of throwing if it never does (including when the site is
    /// genuinely empty — a text-based shortcut for that was tried and reverted: matching "No
    /// active rec..." anywhere on the page is too easy to false-positive on a different tab's
    /// leftover panel, which incorrectly treated a tab WITH real data as empty).
    /// </summary>
    ...
    public void EnrichWithDetails(ResearchItem item)
    {
        ...
        if (gridRoot == null)
        {
            Console.WriteLine($"  Grid never appeared within 100s — skipping detail fetch for {item.Symbol}.");
            return;
        }
